...
/**
 * A collection of pre-defined animations which are ready-to-use together with
 * the <code>animate</code> method.
 */
qx.Bootstrap.define("baselib.Animation", {

  statics : {
    /** The default values for all named animations */
    __defaultAnimationDesc : {
      duration: 400,
      timing: "linear",
      keep: 100
    },

    /** values are the keyFrame to set */
    __namedAnimations : {
      "GROW" : 100,
      "GROW_WIDTH" : 100,
      "GROW_HEIGHT" : 100,
    ...
    /** CSS style attributes to store to be able to run the animation on a shrinked element */
    __valuesToStore : [ "width", "height", "fontSize", "paddingLeft", "paddingTop", "paddingRight", "paddingBottom" ],


    /**
     * Hide the elements with a slide animation.
     *
     * @attach{qxWeb}
     * @param duration {Number} The duration in milliseconds of the animation, which will override the duration given in the description.
     * @param timing {String}  takes one of these predefined values: ease | linear | ease-in | ease-out | ease-in-out (cubic-bezier not available since no CSS animations are used)
     * @return {qxWeb} collection for chaining
     */
    slideUp : function(duration, timing) {
      return this.animateNamed("SHRINK_HEIGHT", duration, timing);
    },

    /**
     * Show the elements with a slide animation.
     *
     * @attach{qxWeb}
     * @param duration {Number} The duration in milliseconds of the animation, which will override the duration given in the description.
     * @param timing {String}  takes one of these predefined values: ease | linear | ease-in | ease-out | ease-in-out (cubic-bezier not available since no CSS animations are used)
     * @return {qxWeb} collection for chaining
     */
    slideDown : function(duration, timing) {
      return this.animateNamed("GROW_HEIGHT", duration, timing);
    },

    /**
     * Run a pre-defined animation
     *
     * @attach {qxWeb}
     * @param animation {String} the name of the pre-defined animation
     * Currently the following named animations are supported:
     *
     * <ul>
     *   <li>GROW</li>
     *   <li>GROW_HEIGHT</li>
     ...
     * @return {qxWeb} collection for chaining
     */
    animateNamed : function(animation, duration, timing) {

      if (baselib.Animation.__namedAnimations[animation] !== undefined) {
        var animationValues;
        var animationProperties;
        var fixedProperty = null;
        var fixedPropertyValue = null;

        switch (animation) {
          case "GROW":
          case "SHRINK":
            animationProperties = [ "width", "height", "fontSize",
                                    "paddingLeft", "paddingTop", "paddingRight", "paddingBottom" ];
          break;

          case "GROW_WIDTH":
          case "SHRINK_WIDTH":
            animationProperties = [ "width", "fontSize", "paddingLeft", "paddingRight" ];
            fixedProperty = "height";
          break;

          case "GROW_HEIGHT":
          case "SHRINK_HEIGHT":
            animationProperties = [ "height", "fontSize", "paddingTop", "paddingBottom" ];
            fixedProperty = "width";
          break;
        }

        var grow = baselib.Animation.__namedAnimations[animation] === 100;

        baselib.Animation.__storeValues(this);

        animationValues = baselib.Animation.__getAnimationValues(this, animationProperties);
        var animations = baselib.Animation.__getAnimationDesc(this, animationProperties, animationValues);

        // restore the original values
        baselib.Animation.__restore(this, animationProperties);

        var collection = this;
        this.forEach(function(item, index) {
          var qItem = qxWeb(item);

          var animationDesc = qxWeb.object.clone(baselib.Animation.__defaultAnimationDesc);
          animationDesc.keyFrames = animations[index];

          if (!duration) {
            duration = animationDesc.duration;
          }

          // use the user-provided value
          if (timing !== undefined && timing !== "linear") {
            animationDesc.timing = timing;
          }

          // show the current item before starting the animation
          // -> IE8 is only applying animations on visible elements
          qItem.show();

          // if one style is fixed get the computed value to set
          // before the animation starts
          if (fixedProperty !== null) {
            fixedPropertyValue = item.style[fixedProperty];

            var styleValue;
            if (fixedProperty === "height" || fixedProperty === "width") {
              styleValue = qItem["get" + qxWeb.string.firstUp(fixedProperty)]() + "px";
            } else {
              styleValue = qItem.getStyle(fixedProperty);
            }

            qItem.setStyle(fixedProperty, styleValue);
          }

          // add the animation of the opacity to every animation
          var currentOpacityValue = item.style.opacity;
          animationDesc.keyFrames[0].opacity = 0;
          animationDesc.keyFrames[100].opacity = 1;

          var animationMethod = grow ? "animate" : "animateReverse";

          qItem[animationMethod](animationDesc, duration).once("animationEnd", function() {
            if (!grow) {
              qxWeb(item).hide();
            }
            ...
            if (fixedPropertyValue !== null) {
              item.style[fixedProperty] = fixedPropertyValue;
            }

            if (index === (collection.length - 1)) {
              collection.emit("animationEnd");
            }
        ...
        if (qxWeb.env.get("qx.debug") && qxWeb.log) {
          qxWeb.log.debug("' " + animation + "' is not a pre-defined named animation!");
        }
      }
    ...
    /**
     * Utility method to create the descriptions for the animation
     *
     * @param collection {qxWeb} collection to work on
     * @param animationProperties {Array} style properties to use
     * @param animationValues {Array} style property values to use
     * @return {}
     */
    __getAnimationDesc : function (collection, animationProperties, animationValues) {
      var animationDescriptions = [];
      ...
      collection.forEach(function(item, index) {
        ...
        for (var i=0, j=animationProperties.length; i<j; i++) {
          keyFrames[0][animationProperties[i]] = "0px";
          keyFrames[100][animationProperties[i]] = animationValues[index][i];
        }
        animationDescriptions.push(keyFrames);
      });
    ...
    /**
     * Utility method to get the style property values for the animation
     *
     * @param collection {qxWeb} collection to work on
     * @param values {Array} values to use
     * @return {Array} fallback values (instead of using transform 'scale')
     */
    __getAnimationValues : function(collection, values) {
      var animationValues = [];

      collection.forEach(function(item) {
        var itemValues = [];
        var qItem = qxWeb(item);
        var rendered = item.style.display !== "none";
        var styles;
        ...
        for (var i=0, j=values.length; i<j; i++) {
          ...
          if (values[i] === "width") {
            itemValues.push(qItem.getWidth() + "px");
          } else if (values[i] === "height") {
            itemValues.push(qItem.getHeight() + "px");
          }
          else {
            itemValues.push(qItem.getStyle(values[i]));
          }
        }


        animationValues.push(itemValues);

        if (!rendered) {
          qItem.setStyles(styles);
          qItem.hide();
        }
      });
    ...
    /**
     * Utility method to store the element values which can be used e.g.
     * to grow a element after it got shrinked
     *
     * @param collection {qxWeb} collection to work on
     */
    __storeValues : function (collection) {

      var values = baselib.Animation.__valuesToStore;

      collection.forEach(function(item) {
        var qItem = qxWeb(item);
        for (var a=0, b=values.length; a<b; a++) {
          var value;
          if (values[a] === "width") {
            value = (qItem.getWidth() -
                     parseInt(qItem.getStyle("paddingLeft"), 10) -
                     parseInt(qItem.getStyle("paddingRight"), 10)) + "px";
          } else if (values[a] === "height") {
            value = (qItem.getHeight() -
                     parseInt(qItem.getStyle("paddingTop"), 10) -
                     parseInt(qItem.getStyle("paddingBottom"), 10)) + "px";
          } else {
            value = qItem.getStyle(values[a]);
          }

          qItem.setProperty("animation-restore-" + values[a], item.style[values[a]]);
        }
      });
    },

    /**
     * Restore the previously stored style values and apply them to the DOM elements.
     *
     * @param collection {qxWeb} collection to operate on
     * @param styleProperties {Array} style properties to apply to the DOM elements
     */
    __restore : function(collection, styleProperties) {

      collection.forEach(function(item) {
        var qItem = qxWeb(item);

        var restoreValue;
        for (var a=0, b=styleProperties.length; a<b; a++) {
          restoreValue = qItem.getProperty("animation-restore-" + styleProperties[a]);

          item.style[styleProperties[a]] = restoreValue;
        }
      });
    }
  },
  ...
  defer : function(statics) {
    qxWeb.$attach({
      "animateNamed" : statics.animateNamed,
      "slideUp" : statics.slideUp,
      "slideDown" : statics.slideDown
    });
  }
});
